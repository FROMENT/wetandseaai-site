---
title: "About"
summary: "The Wet & Sea & AI project — how it's made, where to find us, how to support it."
ShowReadingTime: false
ShowToc: false
ShowBreadCrumbs: false
hideMeta: true
draft: false
---

## Wet & Sea & AI

A personal monitoring space across four themes: **AI & Work**, **DevOps & Cloud**, **Cybersecurity** and **Foresight**. Short videos and concise articles to keep up with these topics without drowning in them.

## How it's made

The YouTube channel and this blog are **co-authored with [NotebookLM](https://notebooklm.google.com/)**. The flow:

1. I pick the themes and the sources (articles, papers, posts).
2. NotebookLM produces the video and a first draft of the text.
3. A custom assembly pipeline (Python + ffmpeg) finalizes the render and publishes to YouTube + this site.

> **Honest disclaimer: quality is not guaranteed at 100%.** The articles are entry points into a topic, not authoritative publications. They provide a frame, sources, and trails to go further on your own. Think of them as an augmented reading note, not a thoroughly reviewed editorial piece.

## Where to find us

- 🌐 **Main site** — [wst-tech.org](https://wst-tech.org/)
- 📺 **YouTube channel** — [Discover All-In 360](https://www.youtube.com/@DISCOVER-ALLIN360)
- 🎧 **Spotify podcast** — [Wet & Sea & AI](https://open.spotify.com/show/0oYaiUgc05GR03ERhV9uEr)

## Support

If you find the project useful, two ways to encourage what comes next:

- [Buy me a ramen on Ko-fi](https://ko-fi.com/newspace) — direct support for the project
- [Etsy shop WetSeaTech](https://www.etsy.com/shop/WetSeaTech) — original creations

Every donation funds a new article or video.

---

*Pascal Froment — Cybersecurity Program Director.*
